/**
 * @fileoverview Admin lesson editor — "Generate with AI" calls Gemini directly.
 */
...
import { generateLessonWithAI } from '@/services/lessonGenerationService';
...
export default function AdminEditLessonScreen() {
  ...
  useEffect(() => {
    if (isNew) {
      setTitle(''); setSummary(''); setContent(''); setDurationText(''); setVideoText('');
      return;
    }
    if (!existing) return;
    ...
    try {
      ...
      if (isNew) {
        ...
        const res = await withTimeout(
          createLesson({ course_id: courseId, order_index: nextOrder, title: t, description: summary.trim() || null, content: content.trim() || null, video_url: videoParse.value, duration_mins }),
          SAVE_TIMEOUT_MS, 'Save lesson'
        );
        if (res.error) throw new Error(res.error);
      } else {
        const res = await withTimeout(
          updateLesson(lessonId, { title: t, description: summary.trim() || undefined, content: content.trim() || undefined, video_url: videoParse.value, duration_mins: duration_mins ?? undefined }),
          SAVE_TIMEOUT_MS, 'Save lesson'
        );
        if (res.error) throw new Error(res.error);
      }
  ...
  const onGenerateWithAI = async () => {
    ...
    if (!topic) { Alert.alert('Topic required', 'Enter a topic for the lesson.'); return; }
    ...
    try {
      ...
      setDurationText(String(result.lesson.duration_mins));
      setAiModalVisible(false);
      setAiTopic('');
    } catch (e) {
      Alert.alert('Generation failed', e instanceof Error ? e.message : 'Please try again.');
    } finally {
    ...
    <LinearGradient colors={['#D6D6D6', '#D6D6D6']} className="flex-1">
      <SafeAreaView className="flex-1" edges={['top']}>
        ...
        <View className="px-5 py-4 flex-row items-center">
          <TouchableOpacity onPress={goToCourseEdit} className="w-10 h-10 rounded-full bg-earth-900/5 items-center justify-center" activeOpacity={0.8}>
            <ChevronLeft size={22} color="#1c1917" strokeWidth={1.5} />
          </TouchableOpacity>
          <View className="ml-3 flex-1">
            <Text className="text-black text-xl font-light" style={{ letterSpacing: -0.3 }}>{isNew ? 'New lesson' : 'Edit lesson'}</Text>
            <Text className="text-earth-500 text-sm font-light">Admin lesson and quiz editor</Text>
          </View>
          {/* Generate with AI button */}
          <TouchableOpacity
            onPress={() => setAiModalVisible(true)}
            className="flex-row items-center bg-primary-600 px-3.5 py-2 rounded-full"
            activeOpacity={0.85}
          >
            <Sparkles size={15} color="white" strokeWidth={1.5} />
            <Text className="text-white text-xs font-medium ml-1.5">Generate with AI</Text>
          </TouchableOpacity>
        </View>

        {/* Form */}
        <ScrollView className="flex-1 px-5" contentContainerStyle={{ paddingBottom: 28 }} keyboardShouldPersistTaps="handled">
          <Text className="text-earth-500 text-xs font-light uppercase tracking-widest mb-2">Lesson title</Text>
          ...
          <Text className="text-earth-500 text-xs font-light uppercase tracking-widest mb-2">Summary (optional)</Text>
          ...
          <Text className="text-earth-500 text-xs font-light uppercase tracking-widest mb-2">Content</Text>
          ...
          <Text className="text-earth-500 text-xs font-light uppercase tracking-widest mb-2">YouTube link (optional)</Text>
          ...
          <Text className="text-earth-500 text-xs font-light uppercase tracking-widest mb-2">Duration (minutes, optional)</Text>
          ...
          <View className="mt-8 pt-5 border-t border-earth-300/60">
            <Text className="text-earth-900 text-base font-light mb-1" style={{ letterSpacing: -0.2 }}>Lesson quiz</Text>
            <Text className="text-earth-500 text-sm font-light mb-2">Create or edit a quiz in a dedicated quiz editor screen.</Text>
            {isNew ? (
              <Text className="text-earth-400 text-sm font-light">Save this lesson first, then create its quiz.</Text>
            ) : (
              <View>
                <Text className="text-earth-600 text-sm font-light mb-3">
                  {existingQuiz ? `Current quiz: ${existingQuiz.title}` : 'No quiz yet for this lesson.'}
                </Text>
                <Button label={existingQuiz ? 'Edit quiz' : 'Create quiz'} onPress={() => router.push(`/admin/courses/${courseId}/lesson-quiz/${lessonId}`)} fullWidth />
              </View>
            )}
          </View>
        </ScrollView>
      ...
      {/* ── AI Generation Modal ─────────────────────────────────────────────── */}
      <Modal visible={aiModalVisible} transparent animationType="fade" onRequestClose={() => !isGenerating && setAiModalVisible(false)}>
        <View className="flex-1 bg-black/50 items-center justify-center px-6">
          <View className="bg-white rounded-3xl p-6 w-full max-w-sm">
            <View className="flex-row items-center mb-4">
              <View className="w-9 h-9 rounded-full bg-primary-100 items-center justify-center mr-3">
                <Sparkles size={18} color="#16a34a" strokeWidth={1.5} />
              </View>
            ...
            <Text className="text-earth-600 text-sm font-light mb-2">
              What topic should this lesson cover?
            </Text>
            <TextInput
              value={aiTopic}
              onChangeText={setAiTopic}
              placeholder={`e.g. "Soil pH and crop yields"`}
              placeholderTextColor="#a8a29e"
              className="border-b border-earth-400/50 py-2.5 text-earth-900 font-light mb-6"
            ...
            {isGenerating ? (
              <View className="items-center py-4">
                <ActivityIndicator size="large" color="#16a34a" />
                <Text className="text-earth-500 text-sm font-light mt-3">Generating lesson…</Text>
              </View>
            ) : (
              <View className="gap-y-3">
                <TouchableOpacity
                  onPress={() => void onGenerateWithAI()}
                ...
                <TouchableOpacity
                  onPress={() => { setAiModalVisible(false); setAiTopic(''); }}

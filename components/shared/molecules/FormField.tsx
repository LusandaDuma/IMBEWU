...
export function FormField({
  ...
  const labelCls = isDark ? 'text-slate-400' : 'text-earth-500';
  const helperCls = isDark ? 'text-slate-500' : 'text-earth-500';
  const iconMuted = isDark ? '#94a3b8' : '#78716c';
  const inputCls = isDark ? `text-white ${className ?? ''}` : `text-earth-800 ${className ?? ''}`;
